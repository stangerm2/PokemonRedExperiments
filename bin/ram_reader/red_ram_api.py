...
class Menus:
    ...
    def get_menu_state(self):
        cursor_location = (self.memory_interface.read_memory(TEXT_MENU_CURSOR_LOCATION[0]),
                           self.memory_interface.read_memory(TEXT_MENU_CURSOR_LOCATION[1]))
        state = TEXT_MENU_CURSOR_LOCATIONS.get(cursor_location, "X")

        text_on_screen = self.memory_interface.read_memory(TEXT_ON_SCREEN)

        if text_on_screen and self.memory_interface.read_memory(0xC490) == 0x79:
            return GameState.TALKING
        elif state != "X" and text_on_screen:
            print(state)

            if self.memory_interface.read_memory(USING_PC_FLAG):
                return GameState.ON_PC
